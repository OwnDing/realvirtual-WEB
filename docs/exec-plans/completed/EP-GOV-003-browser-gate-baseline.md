...
- `--with-deps` 把 Ubuntu APT 外部镜像可用性带入 Browser Gate 的关键路径；run `32216043450` 所用 Ubuntu 24.04 Runner 已预装 Chrome/Chromium 运行环境，因此可固定 OS 并只下载与锁定 Playwright 版本匹配的 Chromium，避免重复刷新系统包索引。
...
- 工作流现在固定使用 Ubuntu 24.04，显式执行 LFS checkout、公共 build、版本匹配的 Chromium 安装和 Browser Harness；安装、测试、job 分别以 10、20、35 分钟上限失败关闭，安装关键路径不再调用 APT。
