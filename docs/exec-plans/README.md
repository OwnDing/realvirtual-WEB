...
- 活动计划放在 [`active/`](active/)，命名 `EP-<AREA>-<NNN>-<slug>.md`。
...
起草时使用 `status: draft, plan_status: proposed`；批准执行后使用 `status: approved, plan_status: active`；完成后保留 `status: approved` 并改为 `plan_status: completed`。
