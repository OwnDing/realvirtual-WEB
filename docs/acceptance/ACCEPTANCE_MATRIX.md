...
| 公开、行业无关 DES | Approved `PS-DES-001`、Accepted `ADR-0005`、Active `EP-DES-001` | 事件队列/runner/组件/快照/实验特征测试迁移到公开门禁；纯通用、物料搬运、涂装线三类黄金流程 | 当前公开 `createDesRunner = null`，`tests/des/` 65 个文件中 53 个公共 DES 测试依赖 `@rv-private/plugins/des/*`，1 个 Toray 项目测试依赖 `@rv-projects/*`；计划已激活，代码实施开始 | active |
