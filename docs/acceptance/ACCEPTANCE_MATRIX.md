...
| Quality Gates | `GOV-HARNESS`、`EP-GOV-003` | GitHub Actions workflow；名称探测终止、包体积与完整 Browser 回归 | 本地 governance/static/node/build 通过。**2026-08-23（[`EP-GOV-004`](../exec-plans/active/EP-GOV-004-gate-that-gates.md) M1）**：本机 Browser 门禁此前 1,031 文件中 22 失败、10,869 例中 82 失败，而 CI 用同一命令零测试失败——差异根因是 Playwright 默认的 `chromium-headless-shell` 没有 GPU 栈（Linux 上回落到可用软件光栅而不可见，macOS 上回落 SwiftShader-on-Vulkan 并 `BindToCurrentSequence failed`）。`test.browser` 改用 Chrome for Testing 新 headless 后，**本机全量 1,028/1,031 文件、10,860 例通过、零失败**，隔离性能套件 11/11。此前各计划把该现象记为「SwiftShader 上下文耗尽」，归因已更正（单跑同样失败，与并发无关）。CI 侧表现待下次 push 的 Browser Gate 确认。**OD-005 于 2026-08-23 关闭**：`main` 已配置分支保护——五项 Gate 全部为 required checks、`strict`、管理员不可绕过、禁止强推与删除、不要求 PR review（单人仓库无法自审）。`develop` 同日按同一策略保护（两分支配置一致，API 复核）。两个分支自此均不接受直接 push | automated-local-and-remote-enforced |
