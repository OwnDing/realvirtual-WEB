...
| 多语言 | Closed OD-002、Approved `PS-I18N-001`、Accepted `ADR-0001`；`zh-CN`/`en-US`、默认中文、AI 直接翻译 | `tests/i18n-inventory.node.test.ts`（增量债务门禁）、`i18n-runtime`/`i18n-catalog`/`i18n-preboot` Node 套件、`tests/i18n-golden-slice.test.tsx` Browser 套件、`scripts/i18n-verbatim-check.mjs` | Projects Dashboard 黄金切片可切换；本地 static + node（497 例）+ build 通过，入口 chunk 净增 59.8 KB 仍在预算内；完整 Browser 门禁受本机 WebGL 上下文创建失败阻塞，未通过项与译文无关但尚未在有 GPU 的环境复验 | golden-slice-local-browser-gate-blocked |
