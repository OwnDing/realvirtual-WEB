...
| 多语言 | Closed OD-002、Approved `PS-I18N-001`、Accepted `ADR-0001`；`zh-CN`/`en-US`、默认中文、AI 直接翻译 | `tests/i18n-inventory.node.test.ts`（增量债务门禁）、`tests/i18n-test-locale-pin.node.test.ts`（测试 locale 固定）、`i18n-runtime`/`i18n-catalog`/`i18n-preboot` Node 套件、`tests/i18n-golden-slice.test.tsx` + `tests/i18n-settings.test.tsx` Browser 套件、`scripts/i18n-verbatim-check.mjs` | Projects Dashboard 与 Settings 面板可切换（批次 2，2026-08-20）；本地 static + node（500 例）+ build 通过，受门禁债务 1858 → 1537，入口 chunk 累计净增 98.3 KB 仍在预算内（余 130.5 KB）；完整 Browser 门禁受本机 WebGL 上下文创建失败阻塞（22 文件 / 82 例，逐条核对均为渲染器上下文，失败输出中文出现次数为 0），尚未在有 GPU 的环境复验 | batch-2-local-browser-gate-blocked |
