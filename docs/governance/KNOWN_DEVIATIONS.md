...
| --- | --- | --- | --- | --- | --- |
| KD-001 | 用户可见文本仍大量散落在 UI 代码，尚无正式字符串目录、语言运行时和迁移门禁 | `GOV-CONSTITUTION` UI-1/UI-2 | OD-002 已关闭且 `PS-I18N-001` 已批准；`ADR-0001` 接受和 `EP-I18N-001` 激活前不得借机引入框架或批量迁移 | Proposed `EP-I18N-001`：盘点与基线 → 接受运行时 ADR → 黄金切片 → 增量迁移 | open |
