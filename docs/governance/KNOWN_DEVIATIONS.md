...
| --- | --- | --- | --- | --- | --- |
| KD-001 | 用户可见文本仍大量散落在 UI 代码，尚无正式字符串目录、语言回退链和翻译责任 | `GOV-CONSTITUTION` UI-1/UI-2 | OD-002 保持 open；新增文本必须在评审中说明临时集中策略，不得借机引入框架 | Proposed `EP-I18N-001`：盘点与基线 → 关闭 OD-002 → 黄金切片 → 增量迁移 | open |
