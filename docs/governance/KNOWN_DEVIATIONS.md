...
| --- | --- | --- | --- | --- | --- |
| KD-001 | 用户可见文本仍大量散落在 UI 代码，尚无正式字符串目录、语言运行时和迁移门禁 | `GOV-CONSTITUTION` UI-1/UI-2 | OD-002 已关闭、`PS-I18N-001` 已批准、`ADR-0001` 已接受、`EP-I18N-001` 已激活；实施必须留在计划的黄金切片边界内，可重复盘点与增量门禁建立前不得批量迁移 UI 文案 | Active `EP-I18N-001`：盘点与基线 → 黄金切片 → 增量迁移 | open |
