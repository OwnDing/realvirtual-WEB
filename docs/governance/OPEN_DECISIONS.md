...
| OD-003 | 部署、项目、文档、模型、用户和会话配置的可覆盖字段及优先级 | product、architecture | 统一配置 Schema、编辑 UI 和迁移 | closed |
...
- 2026-08-29，用户批准关闭 OD-003：普通值采用 `内置 < 部署 < 项目 < 用户 < 会话`，再由部署快照中的组织/部署 policy 执行锁定、deny 与 capability clamp；组织策略在服务器/交付端生成快照，L1 不设浏览器编辑器；模型/文档只选择项目 `modelProfiles`，不是安全层；客户差异不得以源码 fork 表达。规范为 Approved [`PS-CONFIG-002`](../product-specs/UNIFIED_CONFIGURATION.md)、Accepted [`ADR-0008`](../adr/ADR-0008-unified-configuration.md) 和 [`CONTRACT-UNIFIED-CONFIG-001`](../contracts/UNIFIED_CONFIGURATION.md)，实施已由 Completed [`EP-CONFIG-002`](../exec-plans/completed/EP-CONFIG-002-unified-configuration.md) 交付。
