...
### D10 保留上游 CONNECT 授权查询，但在 UI 中明确其归属
...
- 状态机在时间边界上的表驱动测试：`notAfter` 前后各 1 秒、`grace` 边界、时钟回拨。
