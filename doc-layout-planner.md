...
|---|---|
| **Deployment-configured catalogs** | The application composition root passes explicit `catalogUrls` to `LayoutPlannerPlugin`; this XYvirtual deployment subscribes `library/catalog.json`. |
...
A standard parts library ships with this deployment under
[`public/library/`](public/library/) and is explicitly subscribed by the
application composition root in [`src/main.ts`](src/main.ts), which constructs
`LayoutPlannerPlugin({ catalogUrls: ['library/catalog.json'] })` through the
shared `REALVIRTUAL_LIBRARY_PATH` constant. The plugin's own implicit default
list remains empty: another deployment can choose a different catalog or no
catalog without inheriting a hidden fallback. The catalog is generated by
...
If the configured fetch or parse fails, the catalog tab keeps an error state;
there is no silent bundled or remote fallback. `public/models/library/` is not
the standard library home.

The library appears as a catalog tab named *realvirtual Library*, and thumbnails
are rendered at runtime.

Additional catalogs are **opt-in**. Append a
