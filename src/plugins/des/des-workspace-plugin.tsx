 ...
 *     active kernel. The public event-queue overlay exposes live diagnostics.
 *
...
export class DESWorkspacePlugin implements RVViewerPlugin {
   ...
   * runner is unavailable — guarded for optional custom distributions.
   */
  onModeActivate(_mode: ModeId, viewer: RVViewer): void {
    this._syncKernelToDes(viewer);
    // ModeManager invokes this hook just before committing activeMode, so pass
    // the destination explicitly instead of observing the still-old value.
    new DESHMIPlugin().ensureViewer(viewer, _mode);
  ...
  onActivate(viewer: RVViewer): void {
    ...
    new DESHMIPlugin().ensureViewer(viewer, 'des');
    this._syncKernelToDes(viewer);
  }
